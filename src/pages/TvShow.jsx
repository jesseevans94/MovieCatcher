...
const HomePage = () => {
    ...
    const loadTrendMovies = (pageNumber) => {
        const url = `https://api.themoviedb.org/3/trending/tv/day?language=en-US&page=${pageNumber}`;
        const options = {
            method: 'GET',
            headers: {
    ...
    const loadSearchMovies = (pageNumber) => {
        const url = `https://api.themoviedb.org/3/search/tv?query=${inputValue}&include_adult=false/movie/day?language=en-US&page=${pageNumber}`;
        const options = {
            method: 'GET',
            headers: {
                accept: 'application/json',
    ...
    const createMovieListHandler = () => {
        const url = `https://api.themoviedb.org/3/list?session_id=${localStorage.getItem('SessionID')}`;
        const options = {
            method: 'POST',
            headers: {
                accept: 'application/json',
...
<button onClick={logOut} className="ml-4 text-sm rounded-full bg-gradient-to-r from-gray-500 to-gray-700 hover:from-gray-700 hover:to-gray-900 text-white px-5 py-2 font-semibold transition duration-300 ease-in-out">Logout</button>
            ...
            <div className="max-w-[1640px] mx-auto sm:max-w-screen-xl">
                ...
                {trendMovies.results && <TrendTVList trendMovies={trendMovies.results} />}
            </div>
